...
from grayboxes.plot import plot_isomap, plot_isolines
...
class TestUM(unittest.TestCase):
    ...
    def test4(self):
        ...
        for model_type in model_types:
            ...
            X = grid(20, (-2, 2), (-1, 3))
            Y = noise(White('demo')(x=X,silent=True), relative=10e-2)
            x = [[1, 2]]
            y = model(X=X, Y=Y, x=x, 
#                      trainer='Powell', 
                      trainer='auto', 
                      goal=1e-6, trial=5, 
                      epochs=1000, neurons=[6, 6])
    ...
    def _test5(self):
        ...
        X = grid(10, (-2, 2), (-1, 3))
        Y_exa = White('demo')(x=X, silent=True)
        Y = noise(Y_exa, relative=10e-2)
        X0, X1 = X.T[0], X.T[1]
        ...
        for model_type in model_types:            
            ...
            if model_type.__name__ == 'LightGray':
                ...
                trainer = 'auto'
            y = model(X=X, Y=Y, x=X, silent=0, trainer=trainer, show=0, 
                      goal=1e-5, trials=3, neurons=neurons)
