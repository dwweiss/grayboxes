...
class TestUM(unittest.TestCase):
    def setUp(self):
        s = 'Creates true solution y_tru(X) and adds noise. Target is Y(X)'
        print('-' * len(s) + '\n' + s + '\n' + '-' * len(s))
    ...
    def test1(self):
        s = 'Fits model, compare: y(X) vs y_tru(X)'
        print('-' * len(s) + '\n' + s + '\n' + '-' * len(s))
        ...
        plot_x_y_y_ref(self.X, y_lgr, self.y_tru, ['X', 'y_{lgr}', 'y_{tru}'])
        plot_x_y_y_ref(self.X, y_mgr, self.y_tru, ['X', 'y_{mgr}', 'y_{tru}'])
